...
#----------------------------------------------------------------------#
import getopt,hashlib,http.client,itertools,mimetypes,os,pprint,simplejson,sys,time,urllib.parse
#-----------------------------------------------------------------------
key = "put your key here"   # Make sure to set this to your VirusTotal.com public API key.
sleepTime = 30                                                            # VirusTotal.com only allows 20 requests every 5 minitues.
retry = 10                                                                  # The number of times to retry when something fails/Times to wait in the queue.
...
def send_file(filename):
   files = [('file', filename, str(open(filename, 'rb').read()))]
...
def do_files(filenames):
   ...
   for filename in filenames:                                              # Do every file
      try:                                                                 # Keep going even if we get an error
         ...
         for k,v in sorted(reportEntries.items()):
...
def post_multipart(url, fields, files=()):
   ...
   if url_parts.scheme == 'http':
      h = http.client.HTTPConnection(url_parts.netloc)
   elif url_parts.scheme == 'https':
      h = http.client.HTTPSConnection(url_parts.netloc)
   path = urllib.parse.urlunparse(('', '') + url_parts[2:])
